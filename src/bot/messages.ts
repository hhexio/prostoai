export const MESSAGES = {
  WELCOME_NEW: `👋 Привет! Добро пожаловать в <b>ProstoAI</b>!

🤖 Я твой AI-помощник прямо в Telegram.

<b>Что я умею:</b>
💬 Отвечать на любые вопросы (GPT-5.1, GPT-4o)
🎨 Создавать картинки по описанию (Nano Banana 2, DALL-E 3)
📸 Анализировать фото — просто отправь!
🎤 Распознавать голосовые — просто запиши!

🎁 Вам начислено <b>15,000</b> стартовых токенов — попробуйте прямо сейчас!

Просто напиши что угодно — и я отвечу! ✨`,

  WELCOME_BACK: (tokens: number) => `👋 С возвращением!

💰 Ваш баланс: <b>${tokens.toLocaleString('ru-RU')}</b> токенов.

Просто напиши что угодно — и я отвечу! ✨`,

  HELP: `
<b>❓ Справка ProstoAI</b>

<b>Как работают токены?</b>
Токены — внутренняя валюта бота. Каждый запрос к AI списывает токены в зависимости от модели:
• GPT-4.1 Mini — x1 (самый экономный)
• GPT-5 Mini — x1.5
• GPT-4o — x2
• GPT-5.1 — x3.3

<b>Доступные модели:</b>
• 💬 Чат: GPT-4.1 Mini, GPT-5 Mini, GPT-4o, GPT-5.1
• 🎨 Изображения: GPT Image Mini, Nano Banana 2, GPT Image 1, GPT Image 1.5, DALL-E 3
• 📸 Анализ фото: GPT-4o Vision
• 🎤 Голос: Whisper

<b>Как купить токены?</b>
Используйте команду /buy — оплата через Telegram Stars.

<b>Реферальная программа:</b>
/ref — получите ссылку. Каждый приглашённый друг даёт вам и ему по 50 000 токенов!

<b>Команды:</b>
/menu — выбрать модель вручную
/balance — ваш баланс
/buy — купить токены
/ref — реферальная ссылка
/mode — авто/ручной режим
/stats — статистика
/privacy — политика конфиденциальности
/terms — условия использования
`,

  IMAGE_CATEGORY_INFO: `🎨 <b>Генерация изображений:</b>

🖌 Создавайте фотографии по описанию!

💡 <b>Как пользоваться:</b>
• Опишите что хотите увидеть текстом
• Можно на русском или английском
• Чем подробнее описание — тем лучше результат

🍌 Nano Banana 2 также умеет редактировать фото!
Отправьте фото + описание что изменить.

Выберите модель:`,

  NANO_BANANA_SELECTED: `🍌 <b>Nano Banana 2</b> · твори и экспериментируй

🖼 <b>Создавайте:</b>
– Фотографии по текстовому описанию
– Модель отлично наследует исходное фото и может работать с ним

📸 <b>Добавляйте до 5 фото в одном сообщении:</b>
– Добавьте к запросу одно или несколько фото с разными объектами
– Укажите что с ними сделать: соединить, заменить что-то, отредактировать

💡 Отправьте текст или фото!`,

  PHOTO_CATEGORY_INFO: `📸 <b>Работа с фото:</b>

Отправьте мне фото — и я:
• Опишу что на нём изображено
• Отвечу на вопросы о фото
• Помогу с анализом содержимого

💡 Просто отправьте фото с подписью-вопросом или без!`,

  AUDIO_CATEGORY_INFO: `🎤 <b>Работа с аудио:</b>

Отправьте голосовое сообщение — и я распознаю его в текст!

💡 Просто запишите голосовое сообщение.`,

  BALANCE: (tokens: number) => `👤 <b>Ваш профиль</b>

💰 Баланс: <b>${tokens.toLocaleString('ru-RU')}</b> токенов
📊 Хватит примерно на <b>${Math.floor(tokens / 1500)}</b> запросов`,

  BUY_PROMPT: `💎 <b>Выберите пакет токенов:</b>

⏳ Сгорающие (30 дней) — дешевле:
♾ Постоянные — не сгорают:`,

  NO_BALANCE: `💰 <b>Баланс исчерпан!</b>

💎 Купите токены, чтобы продолжить общение с AI.
Токены не сгорают (постоянные пакеты) или действуют 30 дней.

Самый доступный пакет — всего 99₽ ✨`,

  REFERRAL: (link: string, count: number, earned: number) => `🔗 <b>Ваша реферальная ссылка:</b>

<code>${link}</code>

🎁 Пригласите друга — оба получите 50,000 токенов!
Бонус начисляется после первого запроса друга.

📊 Статистика:
• Приглашено друзей: ${count}
• Заработано токенов: ${earned.toLocaleString('ru-RU')}`,

  ERROR_GENERAL: '😔 Что-то пошло не так. Попробуйте ещё раз или выберите другую модель через /menu',
  ERROR_TIMEOUT: '⏱ Запрос занял слишком много времени. Попробуйте позже или выберите другую модель.',
  ERROR_RATE_LIMIT: '⚡ Слишком много запросов. Подождите немного.',
  ERROR_SERVICE: '🔧 AI-сервис временно недоступен. Попробуйте позже.',

  MODEL_SELECTED: (displayName: string) =>
    `✅ Модель переключена на <b>${displayName}</b>.\n\nОтправьте любое сообщение!`,

  AUTO_MODE: '🔄 Переключено в <b>авто-режим</b>. Модель выбирается автоматически под каждый запрос.',

  REFERRAL_BONUS: (tokens: number) =>
    `🎉 Ваш друг зарегистрировался по вашей ссылке! +${tokens.toLocaleString('ru-RU')} токенов на ваш счёт!`,

  PAYMENT_SUCCESS: (tokens: number, packageName: string) =>
    `✅ <b>Оплата прошла успешно!</b>\n\nПакет "${packageName}" активирован.\n+${tokens.toLocaleString('ru-RU')} токенов зачислено на ваш счёт.`,

  PROMO_SUCCESS: (tokens: number) =>
    `🎁 Промокод применён! +${tokens.toLocaleString('ru-RU')} токенов зачислено.`,

  PROMO_INVALID: '❌ Промокод недействителен или уже использован.',

  PRIVACY_POLICY: `📄 <b>Политика конфиденциальности</b>

ProstoAI собирает и хранит:
• Telegram ID и имя пользователя
• Историю запросов и использования моделей
• Данные о платежах (без данных карт)

Мы НЕ передаём ваши данные третьим лицам.
Запросы к AI обрабатываются через ProxyAPI.

Ваши данные используются только для:
• Работы бота и списания токенов
• Улучшения качества сервиса
• Технической поддержки

Для удаления данных напишите /delete_my_data

По вопросам: @hhexio`,

  TERMS_OF_SERVICE: `📋 <b>Условия использования</b>

• ProstoAI предоставляет доступ к AI-моделям
• Стартовый бонус 15,000 токенов при регистрации
• Купленные токены не возвращаются (кроме ошибок сервиса)
• Сгораемые пакеты действуют 30 дней
• Запрещено использовать бота для генерации запрещённого контента
• Сервис предоставляется "как есть"

По вопросам: @hhexio`,

  STATS_USER: (requests: number, tokensSpent: number, favoriteModel: string, daysActive: number, referrals: number) => `
📊 <b>Ваша статистика</b>

• Всего запросов: ${requests}
• Токенов потрачено: ${tokensSpent.toLocaleString('ru-RU')}
• Любимая модель: ${favoriteModel}
• Дней активности: ${daysActive}
• Приглашено друзей: ${referrals}
`,

  STATS_ADMIN: (data: {
    totalUsers: number;
    activeToday: number;
    activeWeek: number;
    activeMonth: number;
    revenueTotal: number;
    revenueToday: number;
    revenueMonth: number;
    requestsToday: number;
    newUsersToday: number;
    topModels: { model: string; count: number }[];
    freeRatio: number;
  }) => `
📊 <b>Статистика платформы</b>

👥 <b>Пользователи:</b>
• Всего: ${data.totalUsers}
• Сегодня новых: ${data.newUsersToday}
• Активны сегодня: ${data.activeToday}
• Активны за неделю: ${data.activeWeek}
• Активны за месяц: ${data.activeMonth}

💰 <b>Выручка:</b>
• Сегодня: ${(data.revenueToday / 100).toFixed(0)} ₽
• За месяц: ${(data.revenueMonth / 100).toFixed(0)} ₽
• Всего: ${(data.revenueTotal / 100).toFixed(0)} ₽

📨 <b>Запросы сегодня:</b> ${data.requestsToday}
🎁 <b>Доля бесплатных:</b> ${data.freeRatio}%

🤖 <b>Топ моделей:</b>
${data.topModels.map((m, i) => `${i + 1}. ${m.model}: ${m.count}`).join('\n')}
`,
};

export const MODEL_DESCRIPTIONS: Record<string, string> = {
  'gpt-4.1-mini': `⚡ <b>GPT-4.1 Mini</b>

📝 Поддерживает: текст
🚀 Быстрая и экономичная модель для простых задач
💰 ~1,500 токенов за запрос

Отправьте текстовое сообщение!`,

  'gpt-5-mini': `🧠 <b>GPT-5 Mini</b>

📝 Поддерживает: текст
⚖️ Баланс скорости и качества — хорош для большинства задач
💰 ~3,000 токенов за запрос

Отправьте текстовое сообщение!`,

  'gpt-4o': `🌟 <b>GPT-4o</b>

📝 Поддерживает: текст
🎯 Универсальная модель — текст, код, анализ
💰 ~4,000 токенов за запрос

Отправьте текстовое сообщение!`,

  'gpt-5.1': `🔥 <b>GPT-5.1</b>

📝 Поддерживает: текст
🧠 Самая умная модель — сложные задачи, код, глубокий анализ
💰 ~6,600 токенов за запрос

Отправьте текстовое сообщение!`,

  'gpt-image-1-mini': `🖼 <b>GPT Image Mini</b>

🎨 Генерация изображений по описанию
⚡ Быстрая и экономичная
💰 ~8,000 токенов за генерацию

Опишите что хотите увидеть!`,

  'gpt-image-1': `🎨 <b>GPT Image 1</b>

🎨 Генерация качественных изображений
🖌 Хорошая детализация и точность
💰 ~15,000 токенов за генерацию

Опишите что хотите увидеть!`,

  'gpt-image-1.5': `✨ <b>GPT Image 1.5</b>

🎨 Генерация изображений премиум-качества
🖌 Максимальная детализация и реалистичность
💰 ~30,000 токенов за генерацию

Опишите что хотите увидеть!`,

  'dall-e-3': `🌈 <b>DALL-E 3</b>

🎨 Классическая модель от OpenAI
🖌 Креативный и художественный стиль
💰 ~15,000 токенов за генерацию

Опишите что хотите увидеть!`,
};
